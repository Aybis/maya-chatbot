...
### Completed
- [x] **Test suite (38 tests, all passing)** — pytest + pytest-asyncio with per-run isolated temp SQLite DB (overridden via `DATABASE_URL` env before app import) and `httpx.ASGITransport` in-process client. Coverage: Phase A auth/RBAC/orgs/invitations (register, login, refresh, isolation, role change/remove); B1 prompts CRUD + org isolation; B2 API keys (hash-only storage verified in DB, show-once, revoke→401, `/v1/chat/completions` auth gates, no-provider 502, full path with mocked upstream → "PONG"); B3 audit (recording, filter, admin-gate 403, org scoping); B4 MCP (CRUD, toggle, unreachable-server graceful, mocked `/tools` parsing); B5 sandbox (python/js exec, exit-code, stderr, timeout, lang gating, API-key + JWT auth); B6 admin summary (owner 200, member 403). Added `pytest.ini`, `tests/`, pytest pinned in requirements.txt.
